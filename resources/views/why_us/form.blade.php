...
<div class="box box-info" >
    <div class="box-header with-border">
        <div class="row">
            ...
            <input type="hidden" id="url_back" value="{{ route('why_us') }}">
            <input type="hidden" id="id" value="{{ $id }}">
            <input type="hidden" id="url_upload" value="{{route('upload_file') }}?folder=">
            <input type="hidden" id="url_save" value="{{route('whyus_add') }}?id=">
        </div>
    </div>
    <div class="box-body">
            ...
            <div class="panel panel-info">
               <div class="form-group" >
                   <label for="img" class="col-sm-3 control-label1">Image</label>
                        <div class="col-sm-9">
                           <img class="img-thumbnail upload_file" width="200" height="200" id="profile-pre" name="upload_file" src="@if($id>0){{asset('public/images/why_us/'.$get['img'])}}@else{{asset('public/img/df.jpg')}}@endif" alt="your image" />
                ...
                <div class="panel panel-info">
                    <div class="panel-heading1"></div>     
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="desc" class="col-sm-6 control-label1">Title Number</label>
                                <div class="col-sm-6">
                                    <input type="text" id="title_number" class="form-control" name="title_number" value="@if($id>0){{ $get['title_number'] }} @endif">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="desc" class="col-sm-6 control-label1">Title Description</label>
                                <div class="col-sm-6">
                                    <input type="text" id="title_desc" class="form-control" name="title_desc" value="@if($id>0){{ $get['title_desc'] }} @endif">
                                </div>
                            </div>
                            <div class="form-group">
...
function SaveWarning(){
    ...
    var a           = {};
    a.title         = $('#title').val();
    a.title_number         = $('#title_number').val();
    a.title_desc         = $('#title_desc').val();
    a.desc         = $('#desc').val();
    ...
    var url_upd     = $('#url_upload').val();
      // config Uoload
    var folder      = 'why_us';//folder untuk input gambar
    var table       = 't_why_us';//table database
    var field_name  = 'img';//nama field database yang akan di input
    var obj_id      = 'profile-id';//nama id file input gambar/file
    var refresh     = 'why_us';// url untuk reload page setelah input data
